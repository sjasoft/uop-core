...
from uop.core.async_path import db_interface
from uop.core import services as base
...
class Services(base.Services):
    ...
    async def _application_from_db(self, app_info):
        ...
        return Application(
            app_name, description=description, _id=app_info["_id"], **cleaned_meta
        )
    ...
    async def add_app_to_tenant(self, tenant_id, app_id):
        ...
        if tenant_id not in a_tenants:
            ...
            changes = app_changes.tenantmap_translated(
                tenant["id_mapping"], tenant["_id"]
            )
            await tenants.update(
                {"_id": tenant_id}, {"id_mapping": tenant["id_mapping"]}
            )
    ...
    async def register_tenant(self, tenantname, password, email, is_admin=False):
        tenant = tenantx.tenant(
            tenantname=tenantname, password=password, email=email, isAdmin=is_admin
        )
    ...
    async def create_app_owner(self, app_name, app):
        admin = await self.register_tenant(
            app_name, app_name, "info@conceptwareinc.com", is_admin=True
        )
