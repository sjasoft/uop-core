...
from uop.core import db_interface as base
...
class Interface(base.Interface):
    ...
    async def ensure_schema(self, a_schema):
        ...
        has_changes = (
            changes.has_changes()
        )  # TODO need changes to be async changes or equiv
        if has_changes:
    ...
    async def object_for_url(self, url, record=False):
        ...
        return (
            {"existing": True, "object": await self.get_object(results[0])}
            if results
            else {
                "existing": False,
                "object": await self.create_instance_of(
                    "WebURL", record=record, url=url
                ),
            }
        )
    ...
    async def get_object_roles(self, uuid):
        ...
        data_rev = set(
            await self.related.distinct("role_id", criteria=dict(object_id=uuid))
        )
    ...
    async def modify_associated(
        self, kind, current, future, constructor, do_replace=False
    ):
        future = set(future)
    ...
    def add_object_groups(self, object_id, group_ids):
        ...
        return self.modify_associated(
            "grouped",
            self.get_object_groups(object_id),
            group_ids,
            (lambda group_id: meta.Grouped(assoc_id=group_id, object_id=object_id)),
        )
    ...
    def set_object_groups(self, object_id, group_ids):
        ...
        return self.modify_associated(
            ...
            self.get_object_groups(object_id),
            group_ids,
            (lambda group_id: Grouped(group_id, object_id)),
            True,
        )
    ...
    async def add_group_objects(self, group_id, object_ids):
        ...
        return await self.modify_associated(
            "grouped",
            self.objects_in_group(group_id),
            object_ids,
            (lambda object_id: Grouped(group_id, object_id)),
        )
    ...
    async def set_group_objects(self, group_id, object_ids):
        ...
        return self.modify_associated(
            "grouped",
            await self.objects_in_group(group_id),
            object_ids,
            (lambda object_id: Grouped(group_id, object_id)),
            True,
        )
    ...
    async def add_object_tags(self, object_id, tag_ids):
        ...
        return self.modify_associated(
            "tagged",
            self.get_object_tags(object_id),
            tag_ids,
            (lambda tag_id: Tagged(tag_id, object_id)),
        )
    ...
    async def set_object_tags(self, object_id, tag_ids):
        ...
        return self.modify_associated(
            "tagged",
            await self.get_object_tags(object_id),
            tag_ids,
            (lambda tag_id: Tagged(tag_id, object_id)),
            True,
        )
    ...
    async def add_tag_objects(self, tag_id, object_ids):
        ...
        return self.modify_associated(
            "tagged",
            await self.get_tagset(tag_id),
            object_ids,
            (lambda object_id: Tagged(tag_id, object_id)),
        )
    ...
    async def add_object_related(self, subject, role_id, object_ids):
        ...
        return await self.modify_associated(
            "related",
            await self.get_roleset(subject, role_id),
            object_ids,
    ...
    async def get_object_tags(self, uuid):
        ...
        if not res:
            res = set(
                await self.tagged.find({"object_id": uuid}, only_cols=["associated"])
            )
            if self._cache:
                self._cache.set(key, res)
        return res
    ...
    async def get_object_groups(self, uuid, recursive=False):
        ...
        if not res:
            ...
            async def get_local_groups(item):
                return await self.grouped.find(
                    {"object_id": uuid}, only_cols=["associated"]
                )
