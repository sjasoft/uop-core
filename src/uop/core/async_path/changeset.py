...
import asyncio
from uop.core import changeset as base
...
class ObjectChanges(CrudChanges):
    ...
    async def on_db_delete(self, uuid, collections):
        await asyncio.gather(
            ...
            collections.related.remove(
                {"$or": [{"object_id": uuid}, {"subject_id": uuid}]}
            ),
        )
    ...
    async def delete_from_collections(self, collections, key):
        ...
        await base.RelatedChanges.user_collection(collections).delete_object_references(
            key
        )
        await base.TaggedChanges.user_collection(collections).delete_object_references(
            key
        )
...
class ClassChanges(CrudChanges):
    ...
    async def on_db_delete(self, key, collections):
        ...
        await asyncio.gather(
            ...
            collections.related.remove(
                {"$or": [{"object_id": cls_expr}, {"subject_id": cls_expr}]}
            ),
        )
